 ...
 */ (function(context, $, undefined) {
    ...
    // 프레임웍 이름
    var LIB_NAME = window.LIB_NAME = 'coma';
     ...
     */
    var coma = context[LIB_NAME] || (context[LIB_NAME] = {});
    ...
    extend(coma, {
         ...
         * @example
         * is = 'placeholder' in coma.tmpInput;  // placeholder를 지원하는가
         */
        tmpInput: tmpInput,
     ...
     *
     * ex) coma.namespace('coma.widget.Control', function() {}) 를 네이티브로 풀어서 작성한다면 다음과 같다.
     *
     * var coma = coma || {};
     * coma.ui = coma.ui || {};
     * coma.widget.Control = coma.widget.Control || function() {};
     */
    coma.namespace = function(name, obj) {
        if (typeof name !== 'string') {
            obj && (name = obj);
            return name;
        }
     ...
     * coma.
     */
    coma.define = function(name, object, isExecFn) {
        if (typeof name !== 'string') {
            object = name;
            name = '';
        }
    ...
    coma._prefix = LIB_NAME + '.';

    coma.define( /** @lends coma */ {
        getHost: function() {
         ...
         * coma.browser.isIE6 // IE56
         * coma.browser.isIE7 // IE567
         * coma.browser.isOldIE // IE5678
         * coma.browser.version // IE의 브라우저
         * coma.browser.isChrome // 크롬
         * coma.browser.isGecko // 파이어폭스
         * coma.browser.isMac // 맥OS
         * coma.browser.isAir // 어도비 에어
         * coma.browser.isIDevice // 아이폰, 아이패드
         * coma.browser.isSafari // 사파리
         * coma.browser.isIETri4 // IE엔진
         * coma.browser.isNotSupporte3DTransform // 안드로이드 3.0이하 3d transform지원X
         * coma.browser.isGingerbread // 안드로이드 Gingerbread
         * coma.browser.isIcecreamsandwith // 안드로이드 Icecreamsandwith
         */
        browser: (function() {
            // 아 정리하고 싶당..
            var detect = {},
        ...
        isEmpty: function(value, allowEmptyString) {
            return (value === null) || (value === undefined) || (!allowEmptyString ? value === '' : false) || (coma.is(value, 'array') && value.length === 0);
        },
    ...
    coma.define('string', function() {
        ...
        return /** @lends coma.string */
        {
             ...
             *
             * @example
             * coma.replaceAll("a1b2c3d", /[0-9]/g, ''); => "abcd"
             */
            ...
            format: function(format, val) {
                var args = coma.toArray(arguments).slice(1),
                    isJson = coma.isPlainObject(val);
            ...
            sprintf: (function() {
                ...
                s.d = s.u = function(ins, x) {
                    return coma.number.zeroPad(Number(ins).toString(0x0A), x[2] + x[4], x[3]);
                };
                ...
                s.f = function(ins, x) {
                    ...
                    x[2] = x[2] == "-" ? "+" : "-";
                    return coma.number.zeroPad(ins, x[2] + x[4], x[3]);
                };
     ...
     * @description
     */
    coma.define('uri', /** @lends coma.uri */ {
        ...
        addParam: function(url, string) {
            if (coma.is(string, 'object')) {
                string = coma.object.toQueryString(string);
            }
            if (!coma.isEmpty(string)) {
     ...
     * @description
     */
    coma.define('number', /** @lends coma.number */ {
        /**
    ...
    coma.number.pad = coma.number.zeroPad;
     ...
     * @name coma.array
     */
    coma.define('array', /** @lends coma.array */ {
        /**
         * 배열 병합
        ...
        map: nativeCall(arrayProto.map) || function(obj, cb, ctx) {
            var results = [];
            if (!coma.is(obj, 'array') || !coma.is(cb, 'function')) {
                return results;
            }
            // vanilla js~
            for (var i = 0, len = obj.length; i < len; i++) {
        ...
        every: nativeCall(arrayProto.every) || function(arr, cb, ctx) {
            var isTrue = true;
            if (!coma.is(arr, 'array') || !coma.is(cb, 'function')) {
                return isTrue;
            }
        ...
        include: function(arr, value, b) {
            ...
            return coma.array.indexOf(arr, value, b) > -1;
        },
     ...
     * @name coma.object
     */
    coma.define('object', /** @lends coma.object */ {
         ...
         */
        map: function(obj, cb) {
            if (!coma.is(obj, 'object') || !coma.is(cb, 'function')) {
                return obj;
            }
            var results = {};
            each(obj, function(v, k) {
        ...
        stringify: function(val, opts, pad) {
            ...
            return (function stringify(val, opts, pad) {
                ...
                if ($.isArray(val)) {
                    ...
                    return '[' + opts.nr + coma.array.map(val, function(el, i) {
                        var eol = val.length - 1 === i ? opts.nr : ', ' + opts.nr;
                        return pad + opts.indent + stringify(el, opts, pad + opts.indent) + eol;
                    }).join('') + pad + ']';
                }

                if (coma.isPlainObject(val)) {
                    if (coma.array.indexOf(cache, val) !== -1) {
                        return null;
                    }
                    ...
                    objKeys = coma.object.keys(val);

                    return '{' + opts.nr + coma.array.map(objKeys, function(el, i) {
    ...
    coma.object.has = coma.object.hasItems;
    coma.json = coma.object;
    ...
    coma.define('date', function() {
        ...
        function compare(d1, d2) {
            if (!(d1 instanceof Date)) {
                d1 = coma.date.parse(d1);
            }
            if (!(d2 instanceof Date)) {
                d2 = coma.date.parse(d2);
            }
        ...
        return /** @lends coma.date */
        {
             ...
             *
             * @function
             * @name coma.date.prettyDuration
             * @param {Date|Interval} time 시간
             * @param {Date|Interval} time (Optional) 기준시간
             * @return {JSON}
             *
             * @example
             * coma.date.prettyDuration(new Date() - 51811); -> "52초 이전"
             */
             ...
             *
             * @example
             * coma.date.timeDiff(new Date, new Date(new Date() - 51811));
             */
	...
	* });
     ...
     */
    var Base = (function() {
        var isFn = coma.isFunction,
            emptyFn = coma.emptyFn,
            include = coma.array.include,
            F = function() {},
        ...
        function classExtend(attr, c) {
            ...
            Class.statics = function(o) {
                o = o || {};
                for (var k in o) {
                    if (!coma.array.include(ignoreNames, k)) {
                        this[k] = o[k];
                    }
                }
                return Class;
        ...
        coma.Class = function(attr) {
            return classExtend.apply(this, [attr, true]);
        };
        return coma.Base = Base;
    })();

    coma.define('Env', /** @lends coma */ {
        /**
         * 설정 값들이 들어갈 리터럴
         *
         * @private
     ...
     */
    coma.define('valid', function() {
        var trim = $.trim,
            isString = coma.isString,
            isNumber = coma.isNumber,
            isNumeric = coma.isNumeric,
            isElement = coma.isElement,
        ...
        return /** @lends coma.valid */
        {
            ...
            allowFile: function(path, exts) {
                if (!coma.isArray(exts)) {
                    exts = [exts];
                }
                var ext = coma.string.getFileExt(path);
                if (!ext) {
                    return false;
                }
                return coma.array.include(exts, ext);
            },
            ...
            date: function(str) {
                ...
                return coma.date.isValid(m[1] + '-' + m[2] + '-' + m[3]);
            },
            /**
     ...
     * @description
     */
    coma.define('css3', function() {

        var _tmpDiv = coma.tmpNode,
        ...
        return /** @lends coma.util */
        {
            ...
            pngFix: function() {
                ...
                $('img[@src*=".png"]', document.body).each(function() {
                    ...
                    this.src = '/resource/images/coma/blank.gif';
                });
            ...
            openPopup: function(url, width, height, opts) {
                ...
                coma.browser.isSafari && tmp.push('location=yes');
     ...
     * @name coma.openPopup
     */
    coma.openPopup = coma.util.openPopup;
    ...
    coma.define('Cookie', {
         ...
         * @example
         * coma.cookie.addToArray('arr', 'a');
         * coma.cookie.addToArray('arr', 'b');  // arr:a|b
         */
        addToArray: function(name, val, sep) {
            ...
            if (!coma.array.include(values, val)) {
                values.push(val);
            }
        ...
        removeToArray: function(name, val, sep) {
            ...
            values = coma.array.remove(values, val);
    ...
    coma.define( /** @lends coma */ {
         ...
         *   ...
         * });
         * coma.bindjQuery(Slider, 'hibSlider');
         * // 실제 사용시
        ...
        bindjQuery: function(Klass, name) {
            ...
            $.fn[name] = function(options) {
                ...
                this.each(function() {
                    ...
                    if (typeof options === 'string' && coma.is(instance[options], 'function')) {
                        if (options[0] === '_') {
     ...
     * @returns {*}
     */
    coma.ui = function( /*String*/ name, supr, /*Object*/ attr) {
        var bindName, Klass, isFn = coma.isFunction;
        ...
        coma.define('ui.' + name, Klass);
        if (bindName) {
            coma.bindjQuery(Klass, bindName);
        }
        return Klass;
    };

    coma.ui.setDefaults = function(name, opts) {
        $.extend(true, coma.ui[name].prototype.defaults, opts);
    };

    View = coma.define('ui.View', function() {
        var isFn = coma.isFunction,
         ...
         * });
         *
         * new coma.ui.Slider('#slider', {count: 10});
         */
        var View = coma.Base.extend( /** @lends coma.ui.View# */ {
            $statics: {
            ...
            initialize: function(el, options) {
                ...
                moduleName = me.moduleName = coma.string.toFirstLower(me.name);
                ...
                me.cid = moduleName + '_' + coma.nextSeq(); // 객체 고유 키
                ...
                // }
                me.options.events = coma.extend({},
                execObject(me.events, me),
                execObject(me.options.events, me));
                coma.each(me.options.events, function(value, key) {
                    ...
                    var name = RegExp.$1,
                        selector = RegExp.$2,
                        args = [name],
                        func = isFn(value) ? value : (isFn(me[value]) ? me[value] : coma.emptyFn);
                ...
                me.options.on && coma.each(me.options.on, function(value, key) {
                    me.on(key, value);
                });
            ...
            release: function() {
                ...
                coma.each(me.subViews, function(item, key) {
                    if (key.substr(0, 1) === '$') {
                ...
                coma.array.remove(View._instances, me);
            },
...
(function($, coma, ui, undefined) {
    ...
    var $doc = coma.$doc,
        ...
        strUtil = coma.string,
        dateUtil = coma.date,
        numberUtil = coma.number,
        browser = coma.browser,
    ...
    var Modal = ui('Modal', /** @lends coma.ui.Modal# */ {
        ...
        initialize: function(el, options) {
            ...
            if (me.options.remote) {
                me.$el.load(me.options.remote).done(function() {
                    me.options.show && coma.util.waitImageLoad(me.$el.find('img')).done(function() {
                        me.show();
                    });
                });
            } else {
        ...
        layout: function() {
            var me = this,
                width = 0,
                height = 0,
                winHeight = coma.util.getWinHeight();
        ...
        _draggabled: function() {
            ...
            if (options.dragHandle) {
                ...
                coma.css3.prefix('user-select') && me.$(options.dragHandle).css(coma.css3.prefix('user-select'), 'none');
                me.on('mousedown.modaldrag touchstart.modaldrag', options.dragHandle, function(e) {
                    ...
                    var isMouseDown = true,
                        ...
                        docSize = {
                            width: coma.util.getDocWidth(),
                            height: coma.util.getDocHeight()
                        },
                        oriPos = {
     ...
     * coma.PubSub.trigger('hide:modal')
     */
    coma.PubSub.on('hide:modal', function(e, force) {
        if (force === false) {
            if (Modal.active) {
                Modal.close();
            }
     ...
     */
    coma.PubSub.on('resize:modal', function() {
        if (Modal.active) {
            Modal.active.center();
        }
    });
    ...
    coma.modal = function(el, options) {
        $(el).modal(options);
    };
    ...
    coma.ui.ajaxModal = function() {
        return function(url, options) {
            return $.ajax($.extend({
                ...
                var el = $(html.replace(/\n|\r/g, "")).appendTo('body');
                var modal = new Modal(el, coma.extend({
                    removeOnClose: true,
                    show: true
                }, options));
    ...
    coma.ui.alert = function() {
        ...
        return function(msg, options) {
            ...
            var el = $(coma.ui.alert.tmpl).appendTo('body').find('div.d-content').html(msg).end();
            var modal = new Modal(el, coma.extend({
                removeOnClose: true,
                show: true
            }, options));
    ...
    coma.ui.alert.tmpl = ['<div class="layer_popup small d-alert" role="alert" style="display:none">', '<h1 class="title d-title">알림창</h1>', '<div class="cntt">', '<div class="d-content"> </div>', '<div class="wrap_btn_c">', '<button type="button" class="btn_emphs_small" data-role="ok"><span><span>확인</span></span></button>', '</div>', '</div>', '<button type="button" class="d-close"><span>닫기</span></button>', '<span class="shadow"></span>', '</div>'].join('');
    ///////////////////////////////////////////////////////////////////////////////////////
    ...
    var Calendar = ui('Calendar', /** @lends coma.ui.Calendar# */ {
        ...
        _escape: function() {
            ...
            me.$calendar.add(me.$el).add(me.$input)
                ...
                if (e.keyCode === coma.keyCode.ESCAPE) {
                    me.close();
        ...
        _renderDate: function() {
            var me = this,
                ...
                html = '',
                tmpl = coma.template(opts.template[opts.type] || opts.template.button),
                isHoliday = false,
                isToday = false,
                isOtherMonth = false,
        ...
        _getClassCustomDay: function(y, m, d) {
            ...
            m = m - 1;
            coma.each(me.customDays, function(item, key) {
                var date;
        ...
        setDate: function(date, options) {
            ...
            try {
                var currDate = coma.is(date, 'date') ? date : dateUtil.parse(date);
                if (me.options.header && me.options.useSelectbox) {
                    currDate = me._checkValid(currDate);
                }
        ...
        setHolidays: function(holidays) {
            ...
            if (coma.isArray(holidays)) {
                me.options.holidays = holidays;
            } else if (coma.is(holidays, 'string')) {
            ...
            /*
             ...
             coma.each(me.options.holidays, function(day, i) {
        ...
        setToday: function(today) {
            ...
            if (!coma.is(today, 'date')) {
                try {
                    me.options.today = coma.date.parse(today)
                } catch (e) {
        ...
        prev: function() {
            var me = this,
                currDate = coma.date.add(me.currDate, 'M', - 1);
            if (me.options.header && me.options.useSelectbox) {
                currDate = me._checkValid(currDate);
            }
            me.currDate = currDate;
    ...
    /**
     * @class
     * @name coma.ui.Paginate
     * @description 페이징모듈
     * @extends coma.ui.View
     */
    var Paginate = ui('Paginate', /** @lends coma.ui.Paginate# */ {
        bindjQuery: 'paginate',
        $statics: /** @lends coma.ui.Paginate */
        {
    ...
    /*
     ...
     /////////////////////////////////////////////////////////
     */
    var AjaxList = ui('AjaxList', /** @lends coma.ui.AjaxList# */ {
        defaults: {},
        initialize: function(el, options) {
            var me = this,
                opts;
     ...
     * @class
     * @name coma.ui.Placeholder
     * @extends coma.ui.View
     * @example
     * new coma.ui.Placeholder( $('input[placeholder]'), {});
     * // 혹은 jquery 플러그인 방식으로도 호출 가능
     * $('input[placeholder]').placeholder({});
     */
    var Placeholder = ui('Placeholder', /** @lends coma.ui.Placeholder# */ {
        ...
        initialize: function(el, options) {
            var me = this,
                is = ('placeholder' in coma.tmpInput);
     ...
     * @extends coma.ui.View
     */
    var Selectbox = ui('Selectbox', /** @lends coma.ui.Selectbox# */ {
        ...
        _createLabel: function() {
            ...
            !isTouch && me.$label.on('keydown', function(e) {
                ...
                } else if (e.keyCode === coma.keyCode.DOWN) {
                    me.open();
        ...
        _createList: function() {
            ...
            !isTouch && me.$list.on('keydown', 'li a', function(e) {
                ...
                switch (e.keyCode) {
                    case coma.keyCode.UP:
                        e.stopPropagation();
                        e.preventDefault();
                        items.eq(Math.max(0, index - 1)).children().focus();
                        break;
                    case coma.keyCode.DOWN:
                        e.stopPropagation();
                        e.preventDefault();
        ...
        value: function(_value) {
            ...
            if (arguments.length === 0) {
                ...
                coma.each(me.$el[0].options, function(item, i) {
     ...
     * @extends coma.ui.View
     */
    ui('Tab', /** @lends coma.ui.Tab# */ {
        bindjQuery: 'tab',
        $statics: /** @lends coma.ui.Tab */
        {
...
(function($, coma, ui, undefined) {
    ...
    /**
     * 아코디언  이벤트
     * @name coma.ui.AccordionListEvent
     */
    coma.define("ui.AccordionListEvent",
    /** @lends coma.ui.AccordionListEvent */
    {
     ...
     * 아코디언 모듈
     * @class
     * @name coma.ui.AccordionList
     * @author 김부윤
     */
    ui('AccordionList', /**@lends coma.ui.AccordionList# */ {
        bindjQuery: 'accordion',
        $statics: /**@lends coma.ui.AccordionList */
        {
        ...
        _bindEvent: function() {
            ...
            var clicked = false;
            var isTouch = coma.browser.isTouch;
...
(function($, coma, ui, undefined) {
    ...
    coma.sns = /** @lends coma.sns */
    {
        ...
        { //['facebook', 'twitter', 'kakaotalk', 'kakaostory'/* , 'googleplus'*/],
            ...
            'kakaotalk': /** @lends coma.sns.types.kakaotalk */
            {
                name: '카카오톡',
            ...
            'kakaostory': /** @lends coma.sns.types.kakaostory */
            {
                name: '카카오스토리',
                support: s.APP | s.MOBILE,
                makeParam: function(data) {
            ...
            'line': /** @lends coma.sns.types.line */
            {
                name: '라인',
                support: s.APP | s.MOBILE,
            ...
            'googleplus': /** @lends coma.sns.types.googleplus */
            {
                name: '구글플러스',
                support: s.PC | s.MOBILE,
        ...
        share: function(type, params) {
            ...
            if (!coma.browser.isMobile && service.support & s.MOBILE === 0) {
        ...
        _send: function(type, params) {
            ...
            switch (type) {
                ...
                case 'twitter':
                    if (window.isApp) {
                        coma.app && coma.app.cmd('open_out_webpage', 'link=' + service.baseUrl + service.makeParam(params));
                    } else {
                        window.open(
...
(function($, coma, ui, undefined) {
    ...
    $.fn.buildUIControls = function() {
        ...
        if (!('placeholder' in coma.tmpInput)) { // placeholder
     ...
     * @name coma.GlobalUI
     */
    coma.GlobalUI = /** @lends coma.GlobalUI */
    {
        init: function() {
            if (_isInit) {
                return;
        ...
        link: function() {
            ...
            $doc.on('click.globalui', '[data-control=toggle], .d-toggle', (function() {
                ...
                return function(e) {
                    ...
                    var $el = $(this),
                        target = $el.attr('data-target') || coma.uri.removeHash($el.attr('href')),
        ...
        sns: function() {
            var types = coma.object.keys(coma.sns.types);

            $doc.on('click.globalui', coma.array.map(types, function(val) {
                return '.d-' + val;
            }).join(', '), function(e) {
                e.preventDefault();
    ...
    // 결제가능한 브라우저인가.
    coma.browser.isCanPayment = (function() {
        var b = coma.browser;
...
(function($, coma, ui, undefined) {
        ...
        events: {
            'tabchanged': function() {
                ...
                setTimeout(function() {
                    ...
                    coma.PubSub.trigger('resize:modal'); // ui.Modal 모듈코드 참조
                }, 100);
            }
        }
    });
    ...
    ui.setDefaults('Calendar', {
        ...
        header: false,
        template: {
            button: ['<span class="m-calendar-day" title="<$-title$>">', '<$if(isHoliday){$>', '<span class="hide">휴점일</span>', '<$} if(isToday){$>', '<span class="hide">오늘</span>', '<$}$>', '<a href="#" aria-describedby="d-<$-coma.date.format(date, \'yyyyMMdd\')$>"><$=day$></a>', '</span>'].join(''),
    ...
    $(function() {
        ...
        // 퓨터 패밀리사이트 가기
        !coma.browser.isMobile && $('#footer .family_link .link_go').click(function() {
...
function ssoLink(gateUrl, retUrl, popup, confirmMsg) {
    ...
    if (isRun) {
        $.ajax({
            type: 'GET',
            dataType: 'json',
            url: coma.Env.get("comaTokenUrl"), //'/mycoma/getToken.do',
            success: function(data) {
                ...
                if (window.isApp) {
                    coma.app.cmd("open_out_webpage", "link=" + destUrl);
                } else {
                    if (isPop) {
